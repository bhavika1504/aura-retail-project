# =============================================================
# hardware/dispenser_hardware.py  — Bhavika Mulani (202512079)
#
# Abstract dispenser interface + two concrete implementations.
# HardwareManager can hot-swap between them at runtime without
# any change to the business logic (Path B constraint satisfied).
# =============================================================
import random
import time
...
class SpiralDispenser(DispenserHardware):
    ...
    def dispense(self, product_id: str, qty: int) -> bool:
        ...
        if random.random() < self._failure_rate:
            ...
            return False
        print(f"[SpiralDispenser] ✅ Dispensed {qty}× {product_id}")
...
class RoboticArmDispenser(DispenserHardware):
    ...
    def dispense(self, product_id: str, qty: int) -> bool:
        if random.random() < self._failure_rate:
            ...
            return False
        print(f"[RoboticArmDispenser] ✅ Arm dispensed {qty}× {product_id}")
        time.sleep(0.08)
        return True
